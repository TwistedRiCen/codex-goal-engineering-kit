...
3. Git branch, status, recent relevant history, and current diff;
4. current milestone implementation and affected call chains;
...
If `PLAN.md` conflicts with repository evidence, repository evidence wins. Correct `PLAN.md` to the verified state before continuing. If the repository contains unexplained changes, preserve them and determine ownership before writing overlapping files.

Continue from the first unmet criterion allowed by the current phase. A resumed session must not bypass a discovery, architecture, decision, or acceptance gate.
